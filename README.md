...
- Create two apps on Fly, one for staging and one for production:
  ...
  ```sh
  fly apps create inrisonanza-001a
  fly apps create inrisonanza-001a-staging
  ```
...
- Add a `SESSION_SECRET` to your fly app secrets, to do this you can run the following commands:
  ...
  ```sh
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app inrisonanza-001a
  fly secrets set SESSION_SECRET=$(openssl rand -hex 32) --app inrisonanza-001a-staging
  ```

  > **Note:** When creating the staging secret, you may get a warning from the Fly CLI that looks like this:
  >
  > ```
  > WARN app flag 'inrisonanza-001a-staging' does not match app name in config file 'inrisonanza-001a'
  > ```
  >
...
- Create a database for both your staging and production environments. Run the following:
  ...
  ```sh
  fly postgres create --name inrisonanza-001a-db
  fly postgres attach --app inrisonanza-001a inrisonanza-001a-db

  fly postgres create --name inrisonanza-001a-staging-db
  fly postgres attach --app inrisonanza-001a-staging inrisonanza-001a-staging-db
  ```
